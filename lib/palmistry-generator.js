...
const EMOTIONAL_TRAITS = [
    'глубокая чувствительность, которую вы скрываете от большинства людей',
    'сильная внутренняя интуиция, которой вы не всегда доверяете, но которая редко ошибается',
    'способность видеть людей насквозь — это дар, который вы порой принимаете за должное',
    'редкая эмпатия, которая делает вас опорой для тех, кто вам дорог',
    'природная мудрость, которая часто удивляет даже вас самих',
];
...
function generateReading({ sessionId, name, gender, focusArea, handScore = 0.7 }) {
    ...
    const preview = [
        ...
        '',
        `${heartTrait}. ${genderAdjective(isFemale, 'Вы обладаете', 'Вы обладаете')} ${emotionalTrait}.`,
        '',
    ...
    // Extra traits for full version
    const [trait1, trait2] = pickN(EMOTIONAL_TRAITS, 2, rng);
